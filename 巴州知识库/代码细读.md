...
kbase中score的取值范围是0-1

        # 将 chatchat 的"相似度阈值"转换为 kbase 的"距离阈值"

        # UI 范围是 0-2，假设 kbase 的距离范围也是 0-2

        # 转换公式：kbase_distance_threshold = 2.0 - chatchat_similarity_threshold

        # 这样：chatchat 阈值 2.0（最高）→ kbase 距离 0.0（最严格）→ 返回最少但最相关

        #      chatchat 阈值 0.0（最低）→ kbase 距离 2.0（最宽松）→ 返回最多

        kbase_distance_threshold = 2.0 - score_threshold
