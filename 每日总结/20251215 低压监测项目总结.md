...
本项目主要围绕 **高并发数据处理**、**数据可靠性**和 **复杂业务逻辑** 展开，我的核心贡献集中在以下两个领域：
#### 挑战1：高可靠性文件分帧传输与拼接

- **挑战：录波文件通过 MQS **分帧乱序传输**，必须保证文件完整性、线程安全，同时避免内存泄漏。
- **难点**：帧可能乱序到达、中间帧可能丢失，需要保证完整性与幂等性，避免内存泄漏
- **解决方案**：设计基于`ConcurrentHashMap` + `TreeMap`的线程安全分帧缓存机制
	- **$\text{ConcurrentHashMap}$** 负责管理不同文件的传输状态。    
	- **$\text{TreeMap}$** 用于有序存储接收到的文件帧，支持乱序接收后按序拼接。
	- 实现自动过期清理机制（1小时），避免内存泄漏
	- 通过MD5校验保证文件完整性，数据库唯一索引保证幂等性
	- 文件传输失败不触发整条消息重试，避免帧数据丢失
```java
/**
 * 录波文件分帧缓存（内存实现），用于拼接完整文件。
 */
@Component
public class FileFrameCache {
    private final Map<String, FrameBuffer> cache = new ConcurrentHashMap<>(); # 线程安全的缓存容器
    ...
    
    private static final class FrameBuffer {
        private int expectedFrames;
        private final NavigableMap<Integer, byte[]> frames = new TreeMap<>(); # 有序存储帧数据（支持乱序接收）
        ...
	}        
}
...
- **难点**：平衡批量大小与处理延迟，保证数据一致性
  ...
  - **解决方案**：
	...
	  - 实现**批量收集机制**，将状态量/模拟量数据收集到批次（100-200条/批）
	
	  - 使用MyBatis-Plus的批量插入功能，减少数据库交互次数
	
	  - 同一消息的数据在同一事务中处理，保证数据一致性
...
- **难点**：保证数据最终一致性，服务间解耦
  ...
  - **解决方案**：
	...
	  - 采用消息驱动架构，MQS Consumer负责数据采集和存储，Backend负责数据查询。服务间通过 MQS 和 REST API 通信，实现了高解耦和高弹性。
	
	  - 通过消息队列实现服务间解耦，保证数据最终一致性
	
	  - Backend通过REST API调用Prediction Service，实现服务间通信
	
	  - 使用数据库作为数据源，各服务独立访问，避免直接依赖  
